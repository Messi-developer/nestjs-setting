...
export class ApiGuard implements CanActivate {
    ...
    //     } else if (request.headers['version'] !== '1.0.0') {
    ...
    private async validateBearerToken(request: Request) {
        ...
        const { INT_API_KEY } = process.env;

        const isValidToken = token === INT_API_KEY;

        if (!isValidToken) throw new UnauthorizedException('Invalid Authorization Token');
