...
@Injectable()
export class PortalIntApiService {
    private defaultHeaders = {
        Version: '1.0.0',
        Authorization: 'Bearer ' + process.env.INT_API_KEY,
    }
    ...
    private async sendRequest(req: apiRequest) {
        try {
            ...
            switch (req.method) {
                case 'get':
                    response = await axios.get(`${process.env.INT_API_URL}/${req.url}`, {
                    ...
                    break
                default:
                    response = await axios.post(`${process.env.INT_API_URL}/${req.url}`, req.data, {
                        headers: this.defaultHeaders,
                        responseType: 'json',
                        timeout: 3000,
                    })
            ...
            await this.teamsWebHookService.sendMessage({
                path: `${process.env.INT_API_URL}/${req.url}`,
                response: {
                    status: +200,
